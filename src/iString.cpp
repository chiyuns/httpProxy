	...
	if (nLen > 0)
   	{
		...
		vsnprintf(sTmp, nLen + 1, pcFmt, va);
	}
	va_end(va);
...
int Comm::StringToBitmap(const char *asString,char *pBitsBuff,size_t nBuffSize)
{
     ...
     size_t nStringLen = strlen(asString) ; //?ж????bit
...
string Comm::NumericCharacterEntities(string sStr, const string sSearchWord, const int nBase, char cEndChar)
{
	...
	while(nPos != string::npos)
	{
		...
		a = i;
		if ( (cEndChar!='\0') && (EndStr[0] != cEndChar) || (i == 0))//??????????????????
		{
			sResult += sSearchWord;
		}
		else
		{
			...
			else if ((EndStr[0] == cEndChar) || (EndStr[0] == ';'))	//????????????????????????????????????';'?????????
			{
				sTemp = &EndStr[1];
			}
			else sTemp = &EndStr[0];
